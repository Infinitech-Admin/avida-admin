...
const CareerTable: React.FC = () => {
  ...
  const deleteCategory = async (categoryId: string) => {
    try {
      const headers = getAuthHeaders();
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/careers/${categoryId}`,
        {
          method: "DELETE",
          headers: headers,
        },
  ...
  const filteredCategories = categories.filter(
    (category) =>
      category.referrer &&
      category.referrer.toLowerCase().includes(searchTerm.toLowerCase()),
  );
  ...
  const columns = [
    ...
    {
      label: "Sub Agent",
      accessor: (category: Category) => category.sub_agent,
    },
      ...
      accessor: (category: Category) => (
        <div className="flex gap-2">
          <Link href={`/admin/career/${category.id}`} className="w-full">
            <button
              type="button"
              className="w-full py-1 px-2 gap-x-2 text-xs font-medium rounded-lg border border-transparent bg-gray-500 text-white hover:bg-gray-600 focus:outline-none focus:bg-gray-600 disabled:opacity-50 disabled:pointer-events-none"
            >
